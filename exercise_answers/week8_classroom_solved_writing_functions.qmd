...
```{r}
calculate_volumn <- function(l,w,h) {
  l * w * h
}

calculate_volumn(1,1,1)
```
...
```{r}
age_calculator <- function(x, y=2024) {
  y - x
}
...
r2_calculator(penguins$body_mass_g, penguins$bill_length_mm)
```
...
```{r}
(function(df, pattern) df %>% select(ends_with(pattern)))(penguins, "mm")
```
...
```{r}
how_many_na <- function(data, column) {
  data %>% 
    filter(is.na({{ column }})) %>% 
    nrow()
}
...
```{r}
sorted_mean_mass <- function(data, grouping_col) {
  data %>% 
...
sorted_mean_mass(starwars, homeworld)
```
...
```{r}
sorted_mean_col <- function(data, averaging_col, grouping_col) {
  
  data %>% 
      group_by({{ grouping_col }}) %>% 
      summarize( {{ averaging_col }} := mean({{ averaging_col }}, na.rm = TRUE)) %>% 
      arrange({{averaging_col}})
}

sorted_mean_col(starwars, mass, homeworld)
```
...
sorted_mean_col <- function(data, averaging_col, grouping_col) {
  ...
  data %>% 
      group_by({{ grouping_col }}) %>% 
      summarize(
        "{{ averaging_col }}_mean" := mean({{ averaging_col }}, na.rm = TRUE),
        "{{ averaging_col }}_median" := median({{ averaging_col }}, na.rm = TRUE)
      ) %>% 
      arrange({{averaging_col}})
}
...
```{r}
name_popularity_plotter <- function(data, chosen_name, name_col, pop_col, time_col){
  data %>% 
    filter({{ name_col }} == chosen_name) %>% 
    ggplot(aes(x={{ time_col }}, y={{ pop_col }})) + 
    geom_col(color="orange", fill="orange", alpha=0.5) + theme_classic() +
    labs(title = chosen_name)
}

name_popularity_plotter(babynames, "John", name, prop, year)
name_popularity_plotter(babynames, "John", name, n, year)
```
