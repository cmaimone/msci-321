...
In a Quarto (.qmd) document, the YAML header is the block at the very top of the file that controls:

-   title, author, date
-   output format (HTML, PDF, Word, slides, etc.)
-   global document options (code execution, themes, numbering, etc.)

It is not content — it’s metadata + settings.

To learn more see this guide: <https://quarto-tdg.org/yaml>
